...
- \usepackage{sectsty} \sectionfont{\centering\color{black}\fontsize{18}{15}}
- \usepackage{sectsty} \subsectionfont{\centering\color{black}\fontsize{14}{10}}
...
\raggedright
**Supplementary Text: NAS Data Cleaning and Validation**  
...
$\text{\underline{Results}}$  
\parindent=.35in
  Goad and Chartwell once again failed to search their results for duplicate records. As a result the `r sum(pubs_dupes_summary$n_original)-sum(pubs_dupes_summary$n_deduped)` duplicates that remained in these datasets inflated their estimate of DEI-related publications in Google Scholar and PubMed by `r pubs_dupes_summary %>% filter(dataset=="gs") %>% select(perc_inflated)`% and `r pubs_dupes_summary %>% filter(dataset=="pubmed") %>% select(perc_inflated)`%.  They also failed to exclude hundreds of articles that were published in cultural studies, humanities, and legal journals (Table 4 and ), as well as thousands of non-DEI articles on topics ranging from palliative care for cancer patients to transcatheter aortic valve replacements (see Table S5).  
