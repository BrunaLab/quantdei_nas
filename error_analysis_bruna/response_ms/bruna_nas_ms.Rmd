...
```{r fig1, fig.align='center',fig.cap="(A) Subset of Figure 8 from the NAS report (\\textit{'Fig 8: DEI-related Tweets from all school-related accounts by DEI term'}) with floating y-axes that accentuate negligible increases in very rare terms. (B) The same panels as in A but with identical y-axes scaled by the frequency of the most common term. Note that both of these figures were made with the original, uncorrected NAS data, so the actual number of tweets for each term is much lower." ,fig.height = 8,fig.width=4}
